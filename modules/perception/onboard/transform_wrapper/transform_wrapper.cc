...
bool TransformWrapper::QueryTrans(double timestamp, StampedTransform* trans,
  ...
  std::cout << "----+++ " << frame_id << " ---- " << child_frame_id << std::endl;
  if (!tf2_buffer_->canTransform(frame_id, child_frame_id, query_time,
    ...
    std::cout << "target_time: " << target_time << " latest_buffer_time: " << latest_buffer_time << std::endl; 
    std::cout << "diff time: " << target_time - latest_buffer_time << std::endl;
